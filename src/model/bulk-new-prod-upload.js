...
import { closeBulkProdModel, closeBulkNewProdModel, bulkUploadStart, bulkUploadEnd, bulkUploadProgress, bulkUploadData } from "../actions/model"
import { createBulkProducts, getMerchantProducts, bulkUpdateRequest ,bulkUploadRequest } from "../actions/user"
...
class BulkNewProductupload extends Component {
    ...
    closeBulkProdModel = () => {
        this.props.dispatch(closeBulkNewProdModel())
    ...
    saveAsExcel = async (getData) => {
        ...
        let header = ["Merchant Id", "Product Id", "SKU", "Selling Price", "Activation Status" , "Name"];
    ...
    render() {
        const { bulk_new_prod_show } = this.props
        console.log(this.props,"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
        let inputRef
        return (
            <>
                <Modal show={bulk_new_prod_show} className="bulkupload" style={{backgroundColor:'transparent'}}>
                    <Modal.Header>
                            ...
                            <div className="row">
                                {this.state.fileTaken == false && this.state.uploadUpdatedFile == false ?
                                    <div className="col-md-12 bulk-popup text-center">
                                        {this.state.selected === '' ? (
                                            <>  <h5>Bulk Upload</h5>
                                                <p>Why upload products one-by-one when you can hundreds of products at once </p>
                                        ...
                                        {this.state.products === '' ? (
                                            <>
                                                ...
                                                <br />
                                                {/* <span style={{ cursor: 'pointer' }} className="downloadbulk" ><a href="/bulk_product_upload.xlsx" download >
                                                    Download Template
                                                    </a>
                                                </span> */}
                                            </>
...
function mapStateToProps(state) {
    const { bulk_new_prod_show } = state.model;
...
export default connect(mapStateToProps)(BulkNewProductupload)