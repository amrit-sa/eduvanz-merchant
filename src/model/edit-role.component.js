...
import { closeEditRole } from "../actions/model"
...
class EditRoleModel extends Component {
    ...
    closeEditRole = () => {
        this.props.dispatch(closeEditRole())
    }

  

    componentDidUpdate(prevProps){
        if(prevProps.editrole_show !== this.props.editrole_show)
        {
  ...
  handleAddRole(e) {
    ...
    render() {
        const { editrole_show, user_id, roledata } = this.props;
        console.log("rrr",roledata);
     
       
        return (
            <>
                <Modal show={editrole_show} className="adduser">
                    <Modal.Header>
            ...
            <button type="button" className="adduser close" onClick={this.closeEditRole}> <i className="fas fa-times"></i> </button>
                                <h4 className='ml-3'>Edit Role</h4>   
                            </div>
                            <div>
...
function mapStateToProps(state) {
    const { editrole_show } = state.model;
    const { user_id, sfid, roleid } = state.auth;
...
export default connect(mapStateToProps)(EditRoleModel)